    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                # TODO: Deal larger dims (corresponding to product manifolds)
                ...
                if len(state) == 0:
                    if scaling == "LR":
                        state['l_prev'] = torch.zeros(p.data.shape[0])
                        state['r_prev'] = torch.zeros(p.data.shape[1])
                        state['l_hat_prev'] = torch.zeros(p.data.shape[0])
                        state['r_hat_prev'] = torch.zeros(p.data.shape[1])
                    else:
                        state['vec_prev'] = torch.zeros(p.data.shape)
                        state['vec_hat_prev'] = torch.zeros(p.data.shape)

                rows, cols = p.data.shape[0], p.data.shape[1]
                if scaling == "LR":
                    l_prev, r_prev, l_hat_prev, r_hat_prev = state['l_prev'], state['r_prev'], state['l_hat_prev'], state['r_hat_prev']
                    row_norm, col_norm = (1, 1) if not norm_cov else (1/rows, 1/cols)
                else:
